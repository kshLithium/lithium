...
async function main() {
  ...
  const normalizedPromptComposer = promptComposerSource
    ...
    .replace(
      ...
      "        returnByValue: true,\n        awaitPromise: true,\n        });\n        return Boolean(focusResult.result?.value?.focused);\n    };\n    const waitForImmediateSubmissionSignal = async (timeoutMs) => {\n        const deadline = Date.now() + timeoutMs;\n        const turnSelectorLiteral = JSON.stringify(CONVERSATION_TURN_SELECTOR);\n        const primarySelectorLiteral = JSON.stringify(PROMPT_PRIMARY_SELECTOR);\n        const fallbackSelectorLiteral = JSON.stringify(PROMPT_FALLBACK_SELECTOR);\n        const stopSelectorLiteral = JSON.stringify(STOP_BUTTON_SELECTOR);\n        const inputSelectorsLiteral = JSON.stringify(INPUT_SELECTORS);\n        const sendSelectorsLiteral = JSON.stringify(SEND_BUTTON_SELECTORS);\n        let baseline = typeof deps.baselineTurns === 'number' && Number.isFinite(deps.baselineTurns) && deps.baselineTurns >= 0\n            ? Math.floor(deps.baselineTurns)\n            : null;\n        if (baseline === null) {\n            try {\n                const { result } = await runtime.evaluate({\n                    expression: `document.querySelectorAll(${turnSelectorLiteral}).length`,\n                    returnByValue: true,\n                });\n                const raw = typeof result?.value === 'number' ? result.value : Number(result?.value);\n                if (Number.isFinite(raw)) {\n                    baseline = Math.max(0, Math.floor(raw));\n                }\n            }\n            catch {\n                // ignore; baseline remains unknown\n            }\n        }\n        const baselineLiteral = baseline ?? -1;\n        const script = `(() => {\n      const editor = document.querySelector(${primarySelectorLiteral});\n      const fallback = document.querySelector(${fallbackSelectorLiteral});\n      const turnCount = document.querySelectorAll(${turnSelectorLiteral}).length;\n      const inputSelectors = ${inputSelectorsLiteral};\n      const sendSelectors = ${sendSelectorsLiteral};\n      const readValue = (node) => {\n        if (!node) return '';\n        if (node instanceof HTMLTextAreaElement) return node.value ?? '';\n        return node.innerText ?? '';\n      };\n      const isVisible = (node) => {\n        if (!(node instanceof HTMLElement) || typeof node.getBoundingClientRect !== 'function') return false;\n        const rect = node.getBoundingClientRect();\n        return rect.width > 0 && rect.height > 0;\n      };\n      const isDisabledButton = (node) => {\n        if (!(node instanceof HTMLElement)) return true;\n        const ariaDisabled = node.getAttribute('aria-disabled');\n        const dataDisabled = node.getAttribute('data-disabled');\n        const style = window.getComputedStyle(node);\n        return node.hasAttribute('disabled') || ariaDisabled === 'true' || dataDisabled === 'true' || style.pointerEvents === 'none' || style.display === 'none';\n      };\n      const activeInputs = inputSelectors\n        .map((selector) => document.querySelector(selector))\n        .filter((node) => Boolean(node))\n        .filter((node, index, nodes) => nodes.indexOf(node) === index);\n      const visibleInputs = activeInputs.filter((node) => isVisible(node));\n      const composerInputs = visibleInputs.length > 0 ? visibleInputs : activeInputs;\n      const activeEmpty =\n        composerInputs.length === 0 ? null : composerInputs.every((node) => !String(readValue(node)).trim());\n      const composerCleared = activeEmpty ?? !(String(editor?.innerText ?? '').trim() || String(fallback?.value ?? '').trim());\n      const buttons = sendSelectors\n        .flatMap((selector) => Array.from(document.querySelectorAll(selector)))\n        .filter((node) => node instanceof HTMLElement)\n        .filter((node, index, nodes) => nodes.indexOf(node) === index);\n      const visibleButtons = buttons.filter((node) => isVisible(node));\n      const candidateButtons = visibleButtons.length > 0 ? visibleButtons : buttons;\n      const sendState =\n        candidateButtons.length === 0 ? 'missing' : candidateButtons.some((node) => !isDisabledButton(node)) ? 'ready' : 'disabled';\n      const baseline = ${baselineLiteral};\n      const hasNewTurn = baseline < 0 ? false : turnCount > baseline;\n      return {\n        composerCleared,\n        hasNewTurn,\n        sendState,\n        stopVisible: Boolean(document.querySelector(${stopSelectorLiteral})),\n      };\n    })()`;\n        while (Date.now() < deadline) {\n            const { result } = await runtime.evaluate({ expression: script, returnByValue: true });\n            const info = result?.value ?? {};\n            if (info?.hasNewTurn || info?.stopVisible || (info?.composerCleared && info?.sendState !== 'ready')) {\n                return info;\n            }\n            await delay(100);\n        }\n        return null;\n    };\n    if (!(await focusPrompt())) {"
    )
    .replace(
      ...
      "    const clicked = await attemptSendButton(runtime, logger, deps?.attachmentNames);\n    if (!clicked) {\n        await focusPrompt().catch(() => false);\n        await input.dispatchKeyEvent({\n            type: 'keyDown',\n            ...ENTER_KEY_EVENT,\n            text: ENTER_KEY_TEXT,\n            unmodifiedText: ENTER_KEY_TEXT,\n        });\n        await input.dispatchKeyEvent({\n            type: 'keyUp',\n            ...ENTER_KEY_EVENT,\n        });\n        logger('Submitted prompt via Enter key');\n    }\n    else {\n        logger('Clicked send button');\n    }\n    let submissionSignal = await waitForImmediateSubmissionSignal(4_000);\n    if (!submissionSignal && clicked) {\n        const refocused = await focusPrompt().catch(() => false);\n        if (refocused) {\n            await input.dispatchKeyEvent({\n                type: 'keyDown',\n                ...ENTER_KEY_EVENT,\n                text: ENTER_KEY_TEXT,\n                unmodifiedText: ENTER_KEY_TEXT,\n            });\n            await input.dispatchKeyEvent({\n                type: 'keyUp',\n                ...ENTER_KEY_EVENT,\n            });\n            logger('Send button click did not produce an immediate submission signal; retried via Enter key');\n            submissionSignal = await waitForImmediateSubmissionSignal(4_000);\n        }\n    }\n    if (!submissionSignal) {\n        logger('No immediate submission signal observed; continuing with commit verification');\n    }\n    const commitTimeoutMs = Math.max(60_000, deps.inputTimeoutMs ?? 0);"
    )
    .replace(
      ...
      "    const isVisible = (node) => {\n      if (!(node instanceof HTMLElement) || typeof node.getBoundingClientRect !== 'function') return false;\n      const rect = node.getBoundingClientRect();\n      return rect.width > 0 && rect.height > 0;\n    };\n    const normalize = (value) => (value ?? '').toLowerCase().replace(/\\\\s+/g, ' ').trim();\n    const isDisabled = (button) => {\n      const ariaDisabled = button.getAttribute('aria-disabled');\n      const dataDisabled = button.getAttribute('data-disabled');\n      const style = window.getComputedStyle(button);\n      return (\n        button.hasAttribute('disabled') ||\n        ariaDisabled === 'true' ||\n        dataDisabled === 'true' ||\n        style.pointerEvents === 'none' ||\n        style.display === 'none'\n      );\n    };\n    const candidates = selectors\n      .flatMap((selector) =>\n        Array.from(document.querySelectorAll(selector)).map((node) => ({\n          selector,\n          node,\n        })),\n      )\n      .filter((entry) => entry.node instanceof HTMLElement)\n      .filter((entry, index, entries) => entries.findIndex((candidate) => candidate.node === entry.node) === index)\n      .map((entry) => {\n        const testId = normalize(entry.node.getAttribute('data-testid'));\n        const ariaLabel = normalize(entry.node.getAttribute('aria-label'));\n        const text = normalize(entry.node.textContent);\n        const selectorRank = selectors.indexOf(entry.selector);\n        const sendLike =\n          testId.includes('send') ||\n          ariaLabel.includes('send') ||\n          text === 'send' ||\n          text.startsWith('send ');\n        return {\n          ...entry,\n          selectorRank,\n          visible: isVisible(entry.node),\n          disabled: isDisabled(entry.node),\n          sendLike,\n        };\n      });\n    if (candidates.length === 0) return 'missing';\n    const enabled = candidates.filter((entry) => !entry.disabled);\n    if (enabled.length === 0) return 'disabled';\n    const ranked = enabled.sort((left, right) => {\n      const score = (entry) => [\n        entry.visible ? 1 : 0,\n        entry.sendLike ? 1 : 0,\n        entry.selectorRank === 0 ? 1 : 0,\n        -entry.selectorRank,\n      ];\n      const leftScore = score(left);\n      const rightScore = score(right);\n      for (let index = 0; index < leftScore.length; index += 1) {\n        if (leftScore[index] !== rightScore[index]) {\n          return rightScore[index] - leftScore[index];\n        }\n      }\n      return 0;\n    });\n    const button = ranked[0]?.node;\n    if (!button) return 'missing';\n    // Use unified pointer/mouse sequence to satisfy React handlers.\n    dispatchClickSequence(button);"
    )
    .replace(
      "    const readValue = (node) => {\n\t      if (!node) return '';\n\t      if (node instanceof HTMLTextAreaElement) return node.value ?? '';\n\t      return node.innerText ?? '';\n\t    };\n\t    const isVisible = (node) => {",
    ...
    .replace(
      ...
      "    const visibleInputs = inputs.filter((node) => isVisible(node));\n\t    const activeInputs = visibleInputs.length > 0 ? visibleInputs : inputs;\n\t    const sendButtons = ${JSON.stringify(SEND_BUTTON_SELECTORS)}\n\t      .flatMap((selector) => Array.from(document.querySelectorAll(selector)))\n\t      .filter((node) => node instanceof HTMLElement)\n\t      .filter((node, index, nodes) => nodes.indexOf(node) === index);\n\t    const visibleSendButtons = sendButtons.filter((node) => isVisible(node));\n\t    const candidateSendButtons = visibleSendButtons.length > 0 ? visibleSendButtons : sendButtons;\n\t    const sendState =\n\t      candidateSendButtons.length === 0\n\t        ? 'missing'\n\t        : candidateSendButtons.some((node) => !isDisabledButton(node))\n\t          ? 'ready'\n\t          : 'disabled';\n\t    const userMatched ="
    )
    .replace(
      "      composerCleared,\n      inConversation,\n      href,",
      "      composerCleared,\n      inConversation,\n      sendState,\n      href,"
    ...
    .replace(
      ...
      "        const submissionLikelyStarted = (info?.stopVisible ?? false) || info?.sendState === 'disabled' || (info?.composerCleared && info?.sendState === 'missing');\n        const fallbackCommit = info?.composerCleared &&\n            (Boolean(info?.hasNewTurn) || submissionLikelyStarted) &&\n            (submissionLikelyStarted || info?.assistantVisible || info?.inConversation);"
    );
